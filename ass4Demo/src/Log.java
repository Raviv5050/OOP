...
public class Log extends BinaryExpression {
    ...
    public Expression differentiate(String var) {
        /**
         ...
         //calculating the log of the first expression:  log (f(x))
         Expression logFirstExpression = new Log(new Num(BASE_TWO) , getFirstExpression());
         //calculating the log of the second expression:  log (g(x))
         Expression logSecondExpression = new Log(new Num(BASE_TWO) , getSecondExpression());
         //calculating the multiplication of the expression:  log (f(x)) * g'(x)
         Expression firstMultDifferentiate = new Mult(logFirstExpression , secondDifferentiateExpression);
         //calculating the multiplication of the expression:  f'(x) * log(g(x))
         Expression secondMultDifferentiate = new Mult(firstDifferentiateExpression , logSecondExpression);
         //calculating the division of the expression:  (log (f(x)) * g'(x)) / g(x)
         Expression firstDivDifferentiate = new Div(firstMultDifferentiate , getSecondExpression());
         //calculating the division of the expression:  (f'(x) * log(g(x))) / f(x)
         Expression secondDivDifferentiate = new Div(secondMultDifferentiate , getFirstExpression());
         //calculating the difference of firstDivDifferentiate and secondDivDifferentiate:
         // ((log (f(x)) * g'(x)) / g(x)) - ((f'(x) * log(g(x))) / f(x))
         Expression differenceMultiplicationsExpression = new Minus(firstDivDifferentiate , secondDivDifferentiate);
         //calculating the pow of the log of the first expression by 2:  ( log (f(x)) )^2
         Expression logExpressionPow = new Pow(logFirstExpression , new Num(POW_TWO));
         //calculating the division of the expression:
         //  ( ((log (f(x)) * g'(x)) / g(x)) - ((f'(x) * log(g(x))) / f(x)) ) /  ( log (f(x)) )^2
         Expression fullDifferentiateExpression = new Div(differenceMultiplicationsExpression , logExpressionPow);
